...
import br.edu.ifsp.scl.sdm.todolist.controller.TaskViewModel.Companion.TaskViewModelFactory
...
class MainFragment : Fragment(), OnTaskClickListener {
            ...
            if (requestKey == TASK_FRAGMENT_REQUEST_KEY) {
                ...
                task?.also { receivedTask ->
                    taskList.indexOfFirst { it.time == receivedTask.time }.also { position ->
                        if (position != -1) {
                            viewModel.editTask(receivedTask)
                            taskList[position] = receivedTask
                            tasksAdapter.notifyItemChanged(position)
                        } else {
                            viewModel.insertTask(receivedTask)
    ...
    override fun onRemoveTaskMenuItemClick(position: Int) {
        viewModel.removeTask(taskList[position])
